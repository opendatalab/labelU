...
然后，你需要在你本机或云服务器上安装 MySQL 数据库。你可以参考 [MySQL 官方文档](https://dev.mysql.com/downloads/installer/)来安装，或者使用 Docker。
...
LabelU 默认使用内置的 SQLite 数据库。如果你从 `v1.x` 升级，安装最新版本后，先确认使用内置 SQLite 是否能正常启动：

```bash
labelu
```

然后使用 `migrate_to_mysql` 命令将数据从 SQLite 迁移到 MySQL：
